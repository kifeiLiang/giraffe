    ...
    <transition name="fade">
      <div :class="classes" v-if="visible" :style="styles" ref="dom" :k-placement="placement" v-transferDom :data-transfer="transfer">
        ...
        <div class="k-poptip-title" v-if="title">
          <i class="icon-info_warn" v-if="confirm"></i>
          <span>{{title}}</span>
        </div>
        ...
        <div class="k-poptip-footer" v-if="confirm">
          <y-button type="gray" mini @click.stop="cancel">{{cancelText}}</y-button>
          <y-button type="default" mini @click.stop="ok">{{okText}}</y-button>
        </div>
      </div>
    </transition>
  </div>
