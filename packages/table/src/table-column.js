...
export default {
  ...
  computed: {
    owner() {
      ...
      while (parent && !parent.tableId) { // 一级column,父亲是y-table
    ...
    columnOrTableParent() {
      let parent = this.$parent;
      while (parent && !parent.tableId && !parent.columnId) { // 二级column 父亲是column
        parent = parent.$parent;
      }
      return parent;
    }
  ...
  created() {
    ...
    column.renderCell = function(h, data) {
      ...
      return _self.showOverflowTooltip || _self.showTooltipWhenOverflow
        ?  <y-tooltip effect={ this.table.tooltipEffect } placement="bottom-right" ref="tooltip" content={ data.column.label }><div class="cell k-tooltip" style={ {width: (data.column.realWidth || data.column.width) - 1 + 'px'} }>{ renderCell(h, data) }</div></y-tooltip>
