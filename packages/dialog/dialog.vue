            ...
            <div class="k-modal-footer">
              <slot name="footer">
                <div class="k-pull-right" v-if="type=='modal'">
                  <y-button medium type="gray" @click="cancel">{{cancelText}}</y-button>
                  <y-button medium type="default" @click="ok">{{okText}}</y-button>
                </div>
                <div class="k-pull-center" v-if="type=='toast'">
                  <y-button style="width:120px" medium type="default" hollow @click="ok">{{okText}}</y-button>
                </div>
              </slot>
            </div>
          </div>
  ...
  computed: {
    ...
    iconClasses() {
      ...
      return [
        "k-toast-icon",
        {
          [`${icons[this.icon]}`]: icons[this.icon] && this.icon,
          [`${this.icon}`]: !icons[this.icon] && this.icon
        }
      ];
    },
    iconStyles() {
